...
class FCOSMono3DHeadNOCS(AnchorFreeMono3DHead):
    ...
    def predict_by_feat(self,
        ...
        mlvl_points = self.get_points(featmap_sizes, bbox_preds[0].dtype,
                                      bbox_preds[0].device)
        result_list = []
        result_list_2d = [] #follow dab detr pose
    ...
    def _predict_by_feat_single(self,
        ...
        results_2d.bboxes = torch.zeros((labels.shape[0],4))
        yaw = bboxes.yaw
        ...
        def roty(t):
            ...
            c = torch.cos(t)
            s = torch.sin(t)
            rot_mat = torch.eye(3).unsqueeze(0).repeat(t.shape[0],1,1)
            rot_mat [:,0,0]=c
            rot_mat [:,2,2]=c
            rot_mat [:,0,2]=s
            rot_mat [:,2,0]=-s
        ...
        results_2d.rots = roty(-yaw).reshape(-1,9)
        results_2d.poses = bboxes.gravity_center
        results_2d.scores = scores
        results_2d.labels = labels
        results_2d.sizes = bboxes.dims
        results_2d.scales = torch.ones_like(labels)
        results_2d.rots_norm = roty(-yaw).reshape(-1,9)
